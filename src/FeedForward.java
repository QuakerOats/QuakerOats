public class FeedForward extends NeuralNetwork {
	...
	public void fire() {
		...
		for(int k=0; k<=this.getHiddenlayers().length-1;k++){
			for(int i = 0; i<= getHiddenlayers()[k].length-1;i++){
				this.getHiddenlayers()[k][i].activate();
			}
		}
		for(int i = 0; i<=this.getOutputlayer().length-1;i++){
	...
	public void backpropagation(double[] inputs, double[] outputs){
		...
		for(int k= 0; k<=this.getHiddenlayers()[getHiddenlayers().length-1].length-1;k++){
			for(int i = 0; i<=this.getOutputlayer().length - 1;i++){
	    	double deltaweight = this.getHiddenlayers()[getHiddenlayers().length-1][k].getOutputsynapses()[i].getOutputneuron().getNeurondiff()*this.getHiddenlayers()[getHiddenlayers().length-1][k].getActivation();
		...
		for (int k = 0; k <= this.getHiddenlayers().length - 2; k++) {
			...
			double delta = this.getHiddenlayers()[k][i].getActivation()*(1-this.getHiddenlayers()[k][i].getActivation())*s;
		...
		for( int k = 0; k<= this.getHiddenlayers().length - 2;k++){
			for (int i = 0 ; i<=this.getHiddenlayers()[k].length - 1; i++){
				for(int j = 0 ; j<=this.getHiddenlayers()[k+1].length-1;j++){
				double delta = this.getHiddenlayers()[k][i].getOutputsynapses()[j].getWeightdiff() + this.getHiddenlayers()[k][i].getActivation()*this.getHiddenlayers()[k][i].getOutputsynapses()[j].getOutputneuron().getNeurondiff();
